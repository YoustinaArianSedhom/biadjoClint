...
export default {
  ...
  methods: {
    ...
    getAllHistory(Obj, count) {
      if (count > 0) {
        this.$store
          .dispatch("BiadjoList/HistoryTripsUsers", Obj)
          .then(res => {
            if (res.status == 200) {
              if (res.data.length > 0) {
                res.data.forEach(element => {
                  this.Biadjo.push(element);
                });
                Obj.PageNo++;
                this.getAllHistory(Obj, res.data.length);
    ...
    Search() {
      ...
      this.$validator.validateAll().then(result => {
        if (result) {
          ...
          JsonValue.PageNo = 0;
          this.getAllHistory(JsonValue, 1);
        } else {
          this.$vs.loading.close();
          window.showError();
        }
