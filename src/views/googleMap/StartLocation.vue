  ...
  <div>
    <div>
      ...
      <vs-button
        ...
        @click="Close"
        :disabled="StartBiadjoLocation.StLatitude == null"
...
export default {
  ...
  methods: {
    Close() {
      this.$store.commit("BiadjoList/StartLocationData", null);
      this.$emit("CloseStartMap", this.StartBiadjoLocation);
    },
    initLocationSearch() {
      ...
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(position => {
          ...
          debugger;
          if (this.$store.state.BiadjoList.StartLocationData) {
            var latlng = {
              lat: parseFloat(
                this.$store.state.BiadjoList.StartLocationData.StLatitude
              ),
              lng: parseFloat(
                this.$store.state.BiadjoList.StartLocationData.StLongitude
              )
            };
          } else {
            var latlng = {
      ...
      autocomplete.addListener("place_changed", () => {
        ...
        geocoder.geocode({ placeId: place.place_id }, (results, status) => {
          ...
          AutoCompleteDataObj.StLatitude = place.geometry.location.lat();
          AutoCompleteDataObj.StLongitude = place.geometry.location.lng();
          AutoCompleteDataObj.StPlaceId = place.place_id;
          AutoCompleteDataObj.StPlaceDesicription = place.formatted_address;
          place.address_components.forEach(element => {
            if (element.types.includes("country")) {
              AutoCompleteDataObj.StartCountry = element.long_name;
              AutoCompleteDataObj.StartCountryCode = element.short_name;
              // AutoCompleteDataObj.Country = element.long_name;
              // AutoCompleteDataObj.CountryCode = element.short_name;
            }
            if (element.types.includes("administrative_area_level_1")) {
              AutoCompleteDataObj.StartState = element.long_name;
            }
            if (
              element.types.includes("locality") ||
              element.types.includes("administrative_area_level_2")
            ) {
              AutoCompleteDataObj.StartCity = element.long_name;
            }
          });
          this.StartBiadjoLocation = AutoCompleteDataObj;
